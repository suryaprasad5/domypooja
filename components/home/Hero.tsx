...
export default function Hero() {
  ...
  return (
    <section style={{
      ...
      {/* LEFT — content */}
      <div style={{ padding: '60px 5% 60px 8%', position: 'relative', zIndex: 2, animation: 'fadeUp 0.9s ease both' }}>
        <div style={{
        ...
        <p style={{ fontSize: '1.05rem', color: 'var(--text-muted)', lineHeight: 1.75, maxWidth: 480, marginBottom: 38 }}>
        ...
        <div style={{
          ...
          <button onClick={handleBook} style={{
            ...
            borderRadius: 14, fontFamily: 'var(--font-body)', fontSize: '0.9rem',
        ...
        <div style={{ display: 'flex', gap: 36, flexWrap: 'wrap' }}>
          {STATS.map(s => (
            <div key={s.label}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '2.1rem', fontWeight: 700, color: 'var(--saffron)', lineHeight: 1 }}>{s.num}</div>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-faint)', fontWeight: 600, marginTop: 4, letterSpacing: '0.4px' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
